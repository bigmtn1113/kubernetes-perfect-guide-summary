...
### 동시 실행 제어
잡 실행을 할 때 이전 스케줄링한 잡이 이미 종료된 상태라면 명시적으로 지정하지 않아도 동시 실행되지 않고 새로운 잡을 실행하나, 기존 잡이 실행되고 있을 때는 정책으로 새로운 잡을 실행할지 말지 제어 가능

동시 실행에 대한 정책은 spec.concurrencyPolicy에 지정  
- Allow
  - 기본값이며, 이전에 실행된 잡의 상태와 관계없이 새로운 잡을 계속 생성
- Forbid
  - 이전 잡이 실행 중인 경우 신규 잡을 생성하지 않음
- Replace
